...
"""
Script for plotting Confusion Matrices with a unified scientific publication style.
Supports loading from raw prediction CSVs or pre-computed matrix CSVs.
"""
...
def _load_from_matrix(csv_path: Path) -> Tuple[np.ndarray, List[str]]:
    ...
    if df.shape[1] == df.shape[0] + 1:
        ...
        cm = df.iloc[:,1:].to_numpy()
        return cm, labels
    raise ValueError(f"{csv_path} is not an NxN or N x (N+1) confusion matrix CSV")
...
def _load_from_preds(csv_path: Path, y_col: str, yhat_col: str,
    ...
    if y_col not in df.columns or yhat_col not in df.columns:
        raise ValueError(f"CSV missing columns: {y_col} or {yhat_col}")
...
def _normalize_cm(cm: np.ndarray, mode: str) -> np.ndarray:
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        if mode == "true":
            ...
            raise ValueError("normalize only supports none/true/pred/all")
...
def plot_confmat(
    ...
    *,
    ...
    cm_show = cm_show.T  # ✅ Transpose: X=True Label, Y=Pred Label
    is_float = (normalize != "none")
    fmt = f".{fmt_decimals}f" if is_float else "d"

    # ✅ Use fixed colorbar layout
    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    ax.set_box_aspect(1)  # ✅ Critical for square aspect ratio
...
# ============ Main Process ============
def main():
    ap = argparse.ArgumentParser(description="Plot unified style confusion matrix for research papers")
    ap.add_argument("--csv_train", type=str, default=None, help="Path to training set CSV")
    ap.add_argument("--csv_test",  type=str, default=None, help="Path to test set CSV")
    ap.add_argument("--train_matrix", action="store_true", help="Flag if train CSV is already a matrix")
    ap.add_argument("--test_matrix",  action="store_true", help="Flag if test CSV is already a matrix")
    ap.add_argument("--y_col", type=str, default="y_true", help="Column name for true labels")
    ap.add_argument("--yhat_col", type=str, default="y_pred", help="Column name for predicted labels")
    ap.add_argument("--labels", type=str, default=None, help="Comma-separated labels list")
    ap.add_argument("--out_dir", type=str, default="figs", help="Output directory")
    ap.add_argument("--out_train", type=str, default="confmat_train.png", help="Output filename for train plot")
    ap.add_argument("--out_test", type=str, default="confmat_test.png", help="Output filename for test plot")
    ap.add_argument("--train_title", type=str, default=None, help="Title for train plot")
    ap.add_argument("--test_title",  type=str, default=None, help="Title for test plot")
    ap.add_argument("--normalize", type=str, default="none", choices=["none","true","pred","all"], help="Normalization mode")
    ap.add_argument("--cmap", type=str, default="Blues", help="Colormap name or hex color")
