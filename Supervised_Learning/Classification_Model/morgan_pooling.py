...
# Bad whitespace characters to remove, including zero-width space, etc.
_BAD_WS = ["\u200b", "\u200c", "\u200d", "\ufeff"]
# Translation map for Chinese full-width punctuation to standard ASCII
_TRANS = {
    "：": ",", "、": ",", "；": ";", "：": ":", "（": "(", "）": ")",
    "【": "[", "】": "]", "“": '"', "”": '"', "‘": "'", "’": "'",
}
...
def split_multi_smiles(cell: str) -> List[str]:
    ...
    if not base: return []
    # If the whole string is a parsable SMILES, return it directly
    try:
...
class _SmilesStandardizer:
    def __init__(self, use_std: bool):
        ...
        if use_std:
            ...
            # Prefer organic is set to True to choose the largest fragment that is organic
...
def _find_id_cols(df: pd.DataFrame) -> List[str]:
    keys = ["id","name","sample","recipe","formula","number"] # "样品", "配方", "编号" translated
...
def _build_weights_for_row(
    ...
    for idx, col in enumerate(smiles_cols):
        ...
        if ratio_cols is not None and idx < len(ratio_cols):
            ...
            if rname is not None and rname in row:
                try:
                    val = row[rname]; base = 0.0 if np.isnan(val) else float(val)
                except Exception:
                    base = 1.0
    ...
    if w_arr.sum() <= 0:
        # If weights sum to zero, use a uniform weight among columns with valid SMILES strings
...
def run_pooling(
    ...
    if emit_std_cols:
        for col in smiles_cols:
            ...
            def _std_cell(cell):
                ...
                # Join multiple standardized SMILES with '||'
                return "||".join(ts)
    ...
    def _lowdim_from_tokens(tokens: List[str]) -> Dict[str, float]:
        # Aggregation: For multiple molecules in the same cell, take the average
    ...
    for i, row in df.iterrows():
        # 1) Parse SMILES per column -> tokens
        ...
        for col in fp_source_cols:
            ...
            if not isinstance(raw, str):
                ...
                continue
            if emit_std_cols:
                # If using standardized column, tokens are separated by '||'
                tokens = [t for t in raw.split("||") if t.strip()]
            else:
                tokens = split_multi_smiles(raw)
            smi_tokens_per_col.append(tokens)

        # 2) Calculate recipe weights (using ratio columns + polymer amplification factor)
        ws = _build_weights_for_row(row, smiles_cols, polymer_set, ratio_cols=rc_global, alpha=alpha)

        # 3) Calculate fingerprint & low-dimensional features for each column
        fp_list = []
        ...
        # 4) Record failed cells (cell has token but fingerprint is all zeros)
        ...
        # 5) Weight the fingerprints by recipe weights (pooling)
        H = np.stack(fp_list, axis=0) if fp_list else np.zeros((1, nbits), dtype=np.float32)
        # H is (N_cols, N_bits), ws is (N_cols,). ws[:, None] is (N_cols, 1)
        h_mix = (ws[:, None] * H).sum(axis=0) if len(fp_list) else np.zeros(nbits, dtype=np.float32)

        # 6) Weight the low-dimensional features by recipe weights (column-aligned)
        ...
        # 6.1 Optional: Non-weighted statistics at the recipe level (to reflect heterogeneity)
        ...
        # 6.2 Optional: Pairwise Tanimoto similarity statistics of components (based on count vectors)
    ...
    # --- Concatenate low-dimensional features table ---
    if low_list:
...
def parse_args():
    ...
    p.add_argument("--emit_desc", action="store_true", help="Output RDKit molecular descriptors (ratio weighted)")
    p.add_argument("--emit_frags", action="store_true", help="Output hydrogel-related SMARTS fragment counts (ratio weighted)")
    p.add_argument("--emit_indices", action="store_true", help="Output hydrogel-derived indices (ratio weighted)")
    p.add_argument("--emit_mixstats", action="store_true", help="Output recipe-level min/max/var statistics (per column)")
    p.add_argument("--emit_pairwise", action="store_true", help="Output pairwise Tanimoto similarity statistics of components (mean/min/max)")
