...
def main():
    ...
    ap.add_argument("--cv", type=int, default=0, help="Number of k-folds; enable cross-validation if > 0")
    ap.add_argument("--group_col", default="", help="Grouping column name (for GroupKFold, optional)")
    ap.add_argument("--test_size", type=float, default=0.2, help="Hold-out ratio used when cv=0")
    ap.add_argument("--final_cv", type=int, default=10, help="Number of folds for final re-validation")
    args = ap.parse_args()
    ...
    if args.group_col:
        if args.group_col not in df.columns:
            raise ValueError(f"group_col not found in data columns: {args.group_col}")
    ...
    # ====== Automatically Select Best Parameters and Re-run 10-fold CV ======
    ...
    # === Force cast to int to prevent errors like 25.0 ===
    ...
    # ====== Execute 10-fold CV with Best Parameters ======
