...
# ---- Optional: XGBoost (will handle import error automatically) ----
try:
...
def parse_max_features(val, default=0.25):
    ...
    try:
        ...
        raise ValueError(f"Cannot parse --rf_max_features={val!r}, please use 0<float<=1, positive integer, or 'sqrt'/'log2'/'auto'.") from e
...
def parse_min_samples_split(val, default=2):
    ...
    try:
        ...
        raise ValueError(f"Cannot parse rf_min_samples_split={val!r}") from e
    if 0.0 < f <= 1.0:
        return f
...
def default_xgb(seed: int, args=None):
    if not _HAS_XGB:
        raise RuntimeError("xgboost not installed. Please install it or use --model rf")
    defaults = dict(
...
def make_mlp_pipeline(
    ...
    if solver == "lbfgs" and early_stopping:
        print("[warn] solver=lbfgs does not support early_stopping. Automatically disabled.")
...
class RFPlusMLP(BaseEstimator, RegressorMixin):
    """sklearn style wrapper: Fit RF first, then fit MLP on residuals; predict = rf + mlp"""
    ...
    def fit(self, X, y):
        if self.rf is None or self.mlp_pipe is None:
            raise ValueError("RFPlusMLP requires both 'rf' and 'mlp_pipe' sub-models.")
        self.rf.fit(X, y)
...
# ============== Training / Evaluation / Saving (Single Split) ==============
...
def train_and_eval(
    ...
    if not no_perm:
        try:
            # Run in single thread to prevent hanging
            ...
            print("[warn] permutation_importance skipped:", e)
...
# ============== Cross Validation (Variable K, includes per-fold CSV & model export) ==============
...
def cv10_evaluate(
    ...
    for tr_ind, va_ind in splits:
        ...
        # Collect details for aggregated CSV
    ...
    # Export aggregated OOF/Training details
    if save_train_pred:
        # Validation OOF
...
def main():
    ap = argparse.ArgumentParser(description="RF/XGB/MLP/RF+MLP training script based on concatenated feature CSV (with Cross Validation)")
    ap.add_argument("--in_csv", required=True, help="Training feature CSV (generated by concat_features.py)")
    ap.add_argument("--target", required=True, help="Target column name (must exist in in_csv)")
    ...
    ap.add_argument("--drop_cols", default="", help="Columns to drop during training (comma separated), e.g., 'ID,SampleName'")
    ap.add_argument("--test_csv", default=None, help="Optional: Test feature CSV (also generated by concat_features.py)")
    ap.add_argument("--id_cols", default="", help="Columns to pass through to prediction output (comma separated)")

    # Target Transform
    ap.add_argument("--y_log1p", action="store_true", help="Apply log1p to target for training, expm1 for prediction")

    # Grouping (Data Leakage Prevention)
    ap.add_argument("--group_col", default="", help="Grouping column name for GroupKFold (optional)")
    ap.add_argument("--n_splits", type=int, default=5, help="Number of GroupKFold splits (only for single split training)")
    ap.add_argument("--fold_idx", type=int, default=0, help="Which fold index to use as validation set (only for single split training)")

    # Permutation Importance Toggle
    ap.add_argument("--no_perm", action="store_true", help="Disable permutation_importance (faster)")
    ...
    # MLP Tunable Params
    ap.add_argument("--mlp_hidden", default="256,128,64", help="MLP hidden layers, e.g., '256,128,64'")
    ap.add_argument("--mlp_activation", choices=["relu","tanh","logistic","identity"], default="relu")
    ap.add_argument("--mlp_alpha", type=float, default=1e-3, help="L2 regularization coefficient")
    ap.add_argument("--mlp_lr", type=float, default=1e-3, help="Initial learning rate")
    ap.add_argument("--mlp_max_iter", type=int, default=800)
    ap.add_argument("--mlp_early_stop", action="store_true", help="Enable early stopping")
    ap.add_argument("--mlp_val_fraction", type=float, default=0.1, help="Validation fraction for early stopping")
    ap.add_argument("--mlp_batch_size", default="auto", help="'auto' or integer, e.g., 256")
    ap.add_argument("--mlp_solver", choices=["adam","lbfgs","sgd"], default="adam")

    # Cross Validation (Configurable folds)
    ap.add_argument("--cv10", action="store_true", help="Enable Cross Validation evaluation (folds controlled by --cv_folds, default 10)")
    ap.add_argument("--cv_folds", type=int, default=10,
                    help="Number of CV folds (used with --cv10, default 10, can be set to 5 etc.)")

    # Prediction Table Export Control
    ap.add_argument("--save_train_pred", action="store_true",
                    help="Save training phase predictions (valid_predictions.csv / cvK_oof.csv / cvK_train_infold.csv / per-fold CSV)")
    ap.add_argument("--pred_path", default=None, help="Custom test prediction output path (default <save_dir>/test_predictions.csv)")
    ap.add_argument("--oof_path", default=None, help="Custom OOF prediction output path (default <save_dir>/cvK_oof.csv)")

    # RF Hyperparams (Read by default_rf)
    ...
    ap.add_argument('--rf_min_samples_split', type=float, default=2,
                    help="Min samples required to split an internal node; can be int>=2 or float in (0,1] (fraction)")

    args = ap.parse_args()

    save_dir = Path(args.save_dir); save_dir.mkdir(parents=True, exist_ok=True)

    # Read training feature CSV
    df = read_csv_robust(args.in_csv)
    assert args.target in df.columns, f"Target column {args.target} not found in training CSV"
    ...
    # Cross Validation Evaluation (with per-fold CSV and model export)
    if args.cv10:
        cv10_evaluate(
            df_feat=df,
            target=args.target,
    ...
    # Test Prediction (Output test_predictions.csv if --test_csv is provided)
    if args.test_csv:
        ...
        print("[info] Test set predictions saved:", (args.pred_path or str(save_dir / "test_predictions.csv")))


if __name__ == "__main__":
    main()
    # ---- Force clean joblib thread pool ----
    try:
