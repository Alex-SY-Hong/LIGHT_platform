...
def split_multi_smiles(cell: str) -> List[str]:
    ...
    if not base: return []
    # If the whole string is a parsable SMILES, return directly
    try:
...
def autodetect_smiles_cols(df: pd.DataFrame, min_valid_frac: float = 0.2, sample_n: int = 200) -> List[str]:
    ...
    for c in check_cols:
        ...
        for v in vals:
            ...
            for t in toks[:3]:  # Sample a few tokens
                checked += 1
...
def _find_target_col(df: pd.DataFrame) -> Optional[str]:
    candidates = [
        "Young's Modulus (MPa)","Young’s Modulus (MPa)","Youngs Modulus (MPa)",
        "Young's Modulus (kPa)","Young’s Modulus (kPa)","Youngs Modulus (kPa)",
        "Young_Modulus_MPa","Young_Modulus_kPa","Youngs_Modulus_MPa","Youngs_Modulus_KPa","Youngs_Modulus_kPa", 
        "Young's Modulus (kPa) log10", "Swelling Ratio (times)"
    ]
    def norm(s: str) -> str:
...
def _find_id_cols(df: pd.DataFrame) -> List[str]:
    keys = ["id", "name", "sample", "formula", "recipe", "number"]
...
def run_pooling(
    ...
    def _lowdim_from_tokens(tokens: List[str]) -> Dict[str, float]:
        # Aggregation: take the mean for multiple molecules in the same cell
    ...
    for i, row in df.iterrows():
        ...
        # 3) Calculate fingerprints & low-dim features for each column
        fp_list = []
        ...
        # 4) Log failed cells (cell has token but fingerprint is all zeros)
        ...
        # 5) Fingerprints weighted by recipe weights
        ...
        # 6) Low-dim features weighted by recipe weights (column aligned)
        ...
        # 6.1 Optional: Non-weighted statistics at recipe level (reflecting heterogeneity)
        ...
        # 6.2 Optional: Component pairwise Tanimoto similarity statistics (based on count vectors)
    ...
    # —— Concatenate low-dim feature table —— 
    if low_list:
...
def parse_args():
    ...
    p.add_argument("--emit_pairwise", action="store_true", help="Output component pairwise Tanimoto similarity statistics (mean/min/max)")
