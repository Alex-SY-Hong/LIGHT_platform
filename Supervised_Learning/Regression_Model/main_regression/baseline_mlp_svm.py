...
# ---- Optional: XGBoost (will handle import error if not installed) ----
try:
...
def split_xy(df: pd.DataFrame, target: str, drop_cols: List[str]):
    ...
    # ========== 2. Select numerical feature columns ==========
    cols = [
        c for c in df.columns
        if c not in exclude and np.issubdtype(df[c].dtype, np.number)
    ]
    if not cols:
        raise ValueError("No numerical feature columns detected. Please check the input CSV file.")

    # ========== 3. Output log (print once) ==========
    print(f"[INFO] Training with {len(cols)} feature columns (Excluded: {sorted(exclude)})")
...
def parse_max_features(val, default=0.25):
    ...
    try:
        ...
        raise ValueError(f"Unable to parse --rf_max_features={val!r}, please use 0<float<=1, positive int, or 'sqrt'/'log2'/'auto'.") from e
...
# ============== Cross Validation (Variable K, Export Fold CSV & Models) ==============
...
def cv10_evaluate(
    ...
    # Directory for saving models per fold
    ...
    # Export summary details (OOF/Train)
    if save_train_pred:
        # Validation OOF
...
def main():
    ap = argparse.ArgumentParser(description="Training script for RF/XGB/MLP/RF+MLP/SVM based on concatenated feature CSV (including CV)")
    ap.add_argument("--in_csv", required=True, help="Feature CSV for training (generated by concat_features.py)")
    ap.add_argument("--target", required=True, help="Target column name (must exist in in_csv)")
    ap.add_argument("--model", choices=["rf","xgb","mlp","rf+mlp","svm"], default="rf")  # ? Added svm
    ap.add_argument("--save_dir", required=True, help="Output directory")
    ap.add_argument("--valid_size", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--drop_cols", default="", help="Columns to exclude during training (comma-separated), e.g., 'ID,SampleName'")
    ap.add_argument("--test_csv", default=None, help="Optional: Test set feature CSV (also generated by concat_features.py)")
    ...
    ap.add_argument("--n_splits", type=int, default=5, help="Number of splits for GroupKFold (only for single split training)")
    ap.add_argument("--fold_idx", type=int, default=0, help="Index of the fold to use as validation set (only for single split training)")
    ...
    ap.add_argument("--svm_gamma", default="scale", help="'scale' / 'auto' / float")
    ...
    ap.add_argument("--cv_folds", type=int, default=10,
                    help="Number of CV folds (used with --cv10, default 10)")

    # Prediction Table Export Control
    ap.add_argument("--save_train_pred", action="store_true",
                    help="Save training predictions (valid_predictions.csv / cvK_oof.csv / cvK_train_infold.csv / per-fold CSV)")
    ap.add_argument("--pred_path", default=None, help="Custom output path for test set predictions (default <save_dir>/test_predictions.csv)")
    ap.add_argument("--oof_path", default=None, help="Custom output path for OOF predictions (default <save_dir>/cvK_oof.csv)")

    # RF Hyperparams (Can be read by default_rf)
    ...
    ap.add_argument('--rf_min_samples_split', type=float, default=2,
                    help="Min samples required to split an internal node; int>=2 or float (0,1]")
    ...
    # Cross Validation Evaluation (Includes per-fold CSV & model export)
    if args.cv10:
        cv10_evaluate(
            df_feat=df,
            target=args.target,
    ...
    # Test set prediction (Output test_predictions.csv if --test_csv is provided)
    if args.test_csv:
        ...
        print("[info] Test set predictions saved to:", (args.pred_path or str(save_dir / "test_predictions.csv")))
